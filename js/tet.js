...
(() => {
  ...
  // Nếu bạn muốn ẩn cả tablet trong JS luôn thì dùng max-width: 1024px
  const isSmallDevice = () =>
  ...
  // ====== TET SFX (mp3) ======
  const TET_SFX_SRC = asset(
    "img/audio/tieng_trong_mua_lan-www_tiengdong_com.mp3"
  );
  const TET_SFX_KEY = "AMADAS_TET_SFX_PLAYED";

  let tetAudio = null;
  let tetStopTimer = null;
  ...
  function stopTetAudio() {
    if (tetStopTimer) {
      clearTimeout(tetStopTimer);
      tetStopTimer = null;
    }
    if (!tetAudio) return;
    try {
  ...
  function markSfxPlayed() {
    try {
      sessionStorage.setItem(TET_SFX_KEY, "1");
    } catch {}
  }

  function hasPlayedSfx() {
    try {
      return sessionStorage.getItem(TET_SFX_KEY) === "1";
    } catch {
      return false;
    }
  }

  function scheduleStopAfter5s(a) {
    if (tetStopTimer) clearTimeout(tetStopTimer);
    tetStopTimer = window.setTimeout(() => {
      try {
        a.pause();
        a.currentTime = 0;
      } catch {}
  ...
  function initDecor() {
    ...
    // Ẩn decor ở phone/tablet (đồng bộ với CSS bạn đang làm)
    if (isSmallDevice()) {
      removeIfExists(IDS.left);
      removeIfExists(IDS.right);
      return;
