...
(function () {
  ...
  /* =========================================================
   * AMADAS GUARD + WARNING OVERLAY
   * - Mặc định: KHÓA LUÔN
   * - Mở khóa để debug: thêm ?unlock=1
   * ========================================================= */
  ...
  /* =========================================================
   * KEY GUARD (chặn phím) + overlay
   * - Bản mạnh hơn: capture + stopImmediatePropagation
   * ========================================================= */
  function isDevtoolsHotkey(e) {
    const key = (e.key || "").toLowerCase();
    const code = (e.code || "").toLowerCase();
  ...
  function installKeyGuards() {
    ...
    document.addEventListener(
      ...
      function (e) {
        ...
        // Không chặn thao tác bình thường trong input/textarea
        if (isEditableTarget(e.target)) {
          // vẫn chặn DevTools hotkeys
        }
  ...
  function installRightClickGuard() {
    document.addEventListener(
      ...
      function (e) {
        if (!__AMADAS_LOCK__) return;
        if (isEditableTarget(e.target)) return; // cho input/textarea
        e.preventDefault();
        e.stopPropagation();
  ...
  /* =========================================================
   * DEVTOOLS DETECT (overlay sẽ giữ khi DevTools mở)
   * ========================================================= */
