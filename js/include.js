...
(() => {
  ...
  function setActiveNavLink() {
    ...
    if (current === "/") {
      const home = links.find((a) => {
        try {
          return applyAlias(normPath(new URL(a.href, location.href).pathname)) === "/";
        } catch {
          return false;
        }
      });
  ...
  async function initInclude() {
    if (window.__AMADAS_INCLUDE_INITED__) return;
    window.__AMADAS_INCLUDE_INITED__ = true;
  ...
  window.amadasIncludeInit = initInclude;

  // nếu include.js được load muộn (sau DOMContentLoaded) vẫn chạy được
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => {
      initInclude().catch(console.error);
    });
  } else {
    initInclude().catch(console.error);
  }
